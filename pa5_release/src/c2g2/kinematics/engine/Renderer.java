...
import c2g2.kinematics.animation.Animation;
...
public class Renderer {
    ...
    private Joint2D selectedNode;

    public Renderer(Scene scene) {
        mScene = scene;
        mInput = new MouseInput();
        fk = new ForwardKinematics(scene.skeleton); 
        mAnim = new Animation(fk, "src/resources/frames/man.xml");
    ...
    private void init() {
        ...
        Joint2D root = mScene.skeleton.getRoot();
        ArrayList<Joint2D> nodeList = new ArrayList<Joint2D>();
        mScene.skeleton.findChildNode(root, nodeList);
        ...
        glfwSetKeyCallback(window, keyCallback = new GLFWKeyCallback() {
            ...
            public void invoke(long window, int key, int scancode, int action, int mods) {
                if (key == GLFW_KEY_ESCAPE && action == GLFW_RELEASE){
                	...
                	double[] angles = mScene.skeleton.getAngles();
                	int l = angles.length;
                	double[] tempAngles = new double[l];
                	for (int i=0;i<l;i++) {
                		tempAngles[i]=angles[i];
                	}
                	tempAngles[0]=tempAngles[0]-0.1;
                	fk.updateState(tempAngles);    
        ...
        glfwSetCursorPosCallback(window, new GLFWCursorPosCallback() {
            ...
            public void invoke(long arg0, double arg1, double arg2) {
                if (isClicked) {
                	double mouseX = (arg1/WIDTH)*2-1;
                	double mouseY = (HEIGHT - arg2)/HEIGHT*2 - 1; 
                    ...
                    if (selectedNode==null)
                    {
                    	...
                    	for (Joint2D node: nodeList) {
                    		if (node.getChildJoints().size()==0 &&
                    			(mouseX-node.getPos().x)*(mouseX-node.getPos().x)+
                    			(mouseY-node.getPos().y)*(mouseY-node.getPos().y)<=0.01)
                    		{
                    	...
                    	//selectedNode.setPos(mouseX, mouseY);
                    	ik.updateInverse(mouseX, mouseY,selectedNode,nodeList);
